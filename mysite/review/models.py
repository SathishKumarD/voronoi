...
BED_CHOICES = [(x,x) for x in range(1,6)]
BATH_CHOICES = [(1.0*x/2, 1.0*x/2) for x in range(2,8)]
...
class Review(models.Model):
    ...
    content = models.TextField(
        max_length=1024,
        verbose_name=_('Review the house'),
        blank=False,
    )

    recommended = models.NullBooleanField(
        verbose_name=_('Would you recommend it to your friend Sathish?'),
    ...
    lived_from = models.IntegerField(_('From'),
        choices=YEAR_CHOICES, default=datetime.datetime.now().year,null=True)

    lived_to = models.IntegerField(_('To'),
        choices=YEAR_CHOICES, default=datetime.datetime.now().year,null=True)

    laundry_onsite = models.NullBooleanField(
        verbose_name=_('is Laundry onsite?'),
        blank=True, default=1,null=True,
    )

    beds = models.IntegerField(_('Beds'),
        choices=BED_CHOICES, default=1,null=True)

    baths = models.FloatField(_('Baths'),
        choices=BATH_CHOICES, default=1,null=True)

    rent = models.IntegerField(_('Rent'),null=True)
